...
//   @Transactional // this will help that is anything fails then the database rollback happens
//   // automatically
...
//   // all the entries should be gotten by this, also this will not need transactional because it's
//   // only reading data
...
//     return timeEntryRepository.findById(id).orElseThrow(() -> new
// TimeEntryNotFoundException(id));
//   }
...
//     // we only want draft entries to be editable, to maintain data integrity, we don't want users
// to
//     // go and edit entries that have already been added
//     if (entry.getStatus() != TimeEntryStatus.DRAFT) {
//       throw new RuntimeException(
//           "Cannot edit time entry that is already " + entry.getStatus().name());
//     }
...
//     if (entry.getStatus() != TimeEntryStatus.DRAFT) {
//       throw new RuntimeException(
//           "Cannot delete time entry that is already " + entry.getStatus().name());
//     }
