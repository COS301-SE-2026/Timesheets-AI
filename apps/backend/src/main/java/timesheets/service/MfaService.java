...
public class MfaService {
  ...
  public MfaSetupResponse setup(UUID userId) {
    ...
    if (existingMfa == null) {
      userMfa = UserMfa.builder().userId(userId).secretKey(secretKey).isEnabled(false).build();
    } else {
    ...
    String qrCodeUrl = totpUtils.generateQrCodeUrl(secretKey, user.getEmail(), "Timesheets AI");
  ...
  public void verifySetup(UUID userId, String totpCode) {
    UserMfa userMfa =
        userMfaRepository
            .findByUserId(userId)
            .orElseThrow(() -> new BadRequestException("MFA setup has not been started"));
  ...
  public void disable(UUID userId, String password) {
    ...
    UserMfa userMfa =
        userMfaRepository
            .findByUserId(userId)
            .orElseThrow(() -> new BadRequestException("MFA is not configured"));
