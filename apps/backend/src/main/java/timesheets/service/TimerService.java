...
public class TimerService {
    ...
    public TimerService(TimerSessionRepository timerSessionRepository, TimeEntryRepository timeEntryRepository, WorkspaceMemberRepository workspaceMemberRepository, ProjectRepository projectRepository, TaskRepository taskRepository, ProjectMemberRepository projectMemberRepository) {
    ...
    //this will start a new timer, and in our system only one timer is allowed across the entire workspace
    @Transactional
    public TimerSession startTimer(UUID workspaceMemberId, StartTimerRequest request) {

        //I want to check if a memeber exists
        WorkspaceMember member = workspaceMemberRepository.findById(workspaceMemberId).orElseThrow(() -> new ResourceNotFoundException("Workspace member not found"));

        
        UUID userId = member.getUserId(); //gets the ID of the user
        
        List<WorkspaceMember> userMemberships = workspaceMemberRepository.findAllByUserId(userId); //to find all the workspace memberships for this user
        
        
        
        //finds all the workspace member IDs
        List<UUID> workspaceMemberIds = userMemberships.stream().map(WorkspaceMember::getId).collect(Collectors.toList());
        
        
        //should see if they have an timers in any workspace
        Optional<TimerSession> existingActiveTimer = timerSessionRepository.findFirstByWorkspaceMemberIdInAndIsRunningTrue(workspaceMemberIds);
        
        if (existingActiveTimer.isPresent()) {
            TimerSession activeTimer = existingActiveTimer.get();
            throw new ConflictException("Timer already active in another workspace","You already have a running timer. Stop it before starting a new one.", activeTimer.getId()
            );
        }
        ...
        boolean isMember = projectMemberRepository.existsByProjectIdAndWorkspaceMemberId(project.getId(), workspaceMemberId);
        if (!isMember) {
            throw new UnauthorizedException("You are not assigned to this project"); //cause I want to see if they actually have access
        }
        ...
        return timerSessionRepository.save(timerSession); //so I am creating and getting a timer session
    }
    ...
    public TimeEntry stopTimer(UUID workspaceMemberId) {
        ...
        } //so this should subtract the paused duration to see the actual time- I made the mistake of not cosidering this properlly 
