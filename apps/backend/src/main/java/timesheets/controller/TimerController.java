...
public class TimerController {
    ...
    public TimerController(TimerService timerService, ProjectRepository projectRepository, TaskRepository taskRepository) {
    ...
    public ResponseEntity<?> startTimer(@Valid @RequestBody StartTimerRequest request, @RequestHeader("X-Workspace-Member-Id") UUID workspaceMemberId) {
        try {
            TimerSession timer = timerService.startTimer(workspaceMemberId, request); 
            
            
            ActiveTimerResponse response = convertToResponse(timer); //I am converting the entry to DTO for frontend, going to create a helper
            return ResponseEntity.ok(response);
            
        } 
        catch (ConflictException e) {
            return ResponseEntity.status(HttpStatus.CONFLICT).body(new ErrorResponse(e.getUserMessage(), e.getActiveTimerId()));
    ...
    public ResponseEntity<StopTimerResponse> stopTimer(@RequestHeader("X-Workspace-Member-Id") UUID workspaceMemberId) {
    ...
    public ResponseEntity<ActiveTimerResponse> getActiveTimer(@RequestHeader("X-Workspace-Member-Id") UUID workspaceMemberId) {
    ...
    //! so the database has more fields than we want the frontend to know about, so we are not going to send all those to frontend, so we need to only send what frontend needs
    ...
    private ActiveTimerResponse convertToResponse(TimerSession timer) {
        ...
        if (timer.getIsRunning() && timer.getStartedAt() != null) {
            ...
            long minutes = java.time.Duration.between(timer.getStartedAt(), java.time.LocalDateTime.now()).toMinutes();
        ...
        if (timer.getProjectId() != null) {
            Project project = projectRepository.findById(timer.getProjectId()).orElse(null);//I can find the actual prject name!

            if (project != null) {
                projectName = project.getName();
            }
        }
        //previously I wanted to put something loading, but since I can have a stub, and because of JPA I have the basic functions now I can do this yayyy!!
    ...
    private StopTimerResponse convertToStopResponse(TimeEntry timeEntry, UUID timerId) {
        ...
        createdEntry.setDate(timeEntry.getStartTime() != null ? timeEntry.getStartTime().toLocalDate() : LocalDate.now());

        createdEntry.setStartTime(timeEntry.getStartTime() != null ? timeEntry.getStartTime().toLocalTime() : LocalTime.now());
        createdEntry.setEndTime(timeEntry.getEndTime() != null ? timeEntry.getEndTime().toLocalTime() : LocalTime.now());

        createdEntry.setDurationMinutes(timeEntry.getDurationMinutes());
        createdEntry.setStatus(timeEntry.getStatus() != null ? timeEntry.getStatus().toString() : "DRAFT");
        ...
        CreatedTimeEntryResponse.SimpleProject simpleProject = new CreatedTimeEntryResponse.SimpleProject();
