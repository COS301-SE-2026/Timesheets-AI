...
public interface WorkspaceMemberRepository extends JpaRepository<WorkspaceMember, UUID> {
  ...
  // this will find all the workspace memberships for a given user, thinking it can be used for like
  ...
  @Query(
      "SELECT wm FROM WorkspaceMember wm WHERE wm.workspaceId = :workspaceId AND wm.workspaceRoleId = 3")
  ...
  @Query(
      "SELECT wm FROM WorkspaceMember wm WHERE wm.workspaceId = :workspaceId AND wm.workspaceRoleId = 2")
  ...
  @Query(
      "SELECT wm FROM WorkspaceMember wm WHERE wm.workspaceId = :workspaceId AND wm.workspaceRoleId = 1")
  ...
  @Query(
      "SELECT wm.isLocked FROM WorkspaceMember wm WHERE wm.userId = :userId AND wm.workspaceId = :workspaceId")
