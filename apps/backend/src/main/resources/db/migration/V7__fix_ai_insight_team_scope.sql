-- V6__seed_ai_insights_weekly_summary.sql. Seed data covering every insight type x scope x overall/by-project
-- combination, so the /api/insights proxy has something real to read against for every card on the Dashboard/Insights pages before the
-- FastAPI jobs have actually run against live data. Additive only, per team convention (no schema changes, no edits to
