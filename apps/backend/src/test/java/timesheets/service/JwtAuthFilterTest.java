...
// // and exceptions during token processing, to verify that the filter behaves robustly and
// securely
...
//   //     //this test checks that when the HTTP request contains a Bearer token in the
// Authorization
...
//   //     //still allowing the filter chain to continue processing the request without
// interruption
...
//   //     //the JwtAuthFilter's doFilterInternal method correctly extracts the email from the
// token
//   //     //loads the user details using the UserDetailsService, validates the token against the
// user
//   // details
...
// Authorization
//   // header
//   //     //that is not blacklisted but an exception is thrown during token processing (e.g.,
// during
//   // email extraction),
//   //     //the JwtAuthFilter's doFilterInternal method does not set any authentication in the
//   // security
