...
class ReportsServiceTest {
  ...
  void generateProductivityReport_returnsZeroSummaryForNoEntries() {
    // this test checks the generateProductivityReport method to ensure that when there are no time
  ...
  void generateProductivityReport_correctlySummarisesEntries() {
    ...
    ProductivityReportResponse report = reportsService.generateProductivityReport(request, user);
    // asserting that the report summary metrics are correctly calculated based on the provided time
  ...
  void generateProductivityReport_taskBreakdownSortedByHoursDescending() {
    // this test checks the generateProductivityReport method to ensure that the breakdown of hours
    // logged
    // by task in the generated report is sorted in descending order based on the total hours logged
    // for each task
    LocalDateTime start = LocalDateTime.of(2026, 5, 14, 9, 0);
    when(timeEntryRepository.findByUserIdAndDateRange(any(), any(), any()))
        .thenReturn(List.of(entry(taskId1, 120, start), entry(taskId2, 480, start.plusHours(3))));

    ProductivityReportResponse report = reportsService.generateProductivityReport(request, user);
    // asserting that the task breakdown in the generated report is sorted in descending order based
    // on hours logged, confirming that the service correctly orders the tasks in the breakdown to
  ...
  void generateProductivityReport_excludesEntriesWithNullTaskFromTaskBreakdown() {
    ...
    ProductivityReportResponse report = reportsService.generateProductivityReport(request, user);
    // asserting that time entries with a null task ID are excluded from the task breakdown in the
    // generated report
