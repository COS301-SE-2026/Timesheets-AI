...
//on the time entries retrieved from the TimeEntryRepository, and that the report period and generation timestamp are set correctly
...
import static org.assertj.core.api.Assertions.*; //using AssertJ for more fluent assertions in the tests
...
class ReportsServiceTest {
    ...
    @BeforeEach
    void setUp() {
        user = User.builder() //builds sample user with necessary fields for report generation
    ...
    private TimeEntry entry(UUID taskId, int durationMinutes, LocalDateTime start){
        ...
        //and start time, simplifying the creation of test data for the time entries used in the productivity report tests 
    ...
    void generateProductivityReport_returnsZeroSummaryForNoEntries() {
        //this test checks the generateProductivityReport method to ensure that when there are no time entries for the user within the specified date range
        ...
        //verifying that the service correctly handles cases with no data and does not produce erroneous summaries or breakdowns
        ...
        //asserting that the report summary metrics are all zero and that the breakdowns are empty, confirming that the service correctly generates a report with no entries
    ...
    void generateProductivityReport_correctlySummarisesEntries() {
        ...
        when(timeEntryRepository.findByUserIdAndDateRange(any(), any(), any()))
            .thenReturn(List.of(
                entry(taskId1, 480, day1),
                entry(taskId2, 300, day2)
            ));

        ProductivityReportResponse report = reportsService.generateProductivityReport(request, user);
        //asserting that the report summary metrics are correctly calculated based on the provided time entries, confirming that the service accurately summarizes the total hours logged, total entries, tasks worked on, and projects worked on
    ...
    void generateProductivityReport_taskBreakdownSortedByHoursDescending() {
        //this test checks the generateProductivityReport method to ensure that the breakdown of hours logged
        //by task in the generated report is sorted in descending order based on the total hours logged for each task
        LocalDateTime start = LocalDateTime.of(2026, 5, 14, 9, 0);
        when(timeEntryRepository.findByUserIdAndDateRange(any(), any(), any()))
            .thenReturn(List.of(
                entry(taskId1, 120, start),
                entry(taskId2, 480, start.plusHours(3))
            ));

        ProductivityReportResponse report = reportsService.generateProductivityReport(request, user);
            //asserting that the task breakdown in the generated report is sorted in descending order based on hours logged, confirming that the service correctly orders the tasks in the breakdown to show the most time-consuming tasks first
        assertThat(report.getByTask().get(0).getHoursLogged()).isGreaterThan(
            report.getByTask().get(1).getHoursLogged());
    }

    @Test
    void generateProductivityReport_weeklyBreakdownSortedChronologically() {
        //this test checks the generateProductivityReport method to ensure that the breakdown of hours logged
        //by week in the generated report is sorted in chronological order based on the week start date
        LocalDateTime week20 = LocalDateTime.of(2026, 5, 14, 9, 0);
        LocalDateTime week19 = LocalDateTime.of(2026, 5, 7, 9, 0);

        when(timeEntryRepository.findByUserIdAndDateRange(any(), any(), any()))
            .thenReturn(List.of(
                entry(taskId1, 60, week20),
                entry(taskId1, 60, week19)
            ));

        ProductivityReportResponse report = reportsService.generateProductivityReport(request, user);
        //asserting that the weekly breakdown in the generated report is sorted in chronological order
        ...
        assertThat(report.getByWeek().get(0).getWeek()).isLessThan(
            report.getByWeek().get(1).getWeek());
    }
    ...
    void generateProductivityReport_excludesEntriesWithNullTaskFromTaskBreakdown() {
        ...
        ProductivityReportResponse report = reportsService.generateProductivityReport(request, user);
        //asserting that time entries with a null task ID are excluded from the task breakdown in the generated report
        //confirming that the service correctly filters out entries without associated tasks when generating the breakdown by task
