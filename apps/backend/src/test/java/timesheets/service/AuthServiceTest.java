...
// // operations such as
// // registration, email verification, login, logout, and password reset in the timesheets
// application
...
// // scenarios, and edge
// // cases. Each test method corresponds to a specific functionality of the AuthService, ensuring
// that
// // the service behaves as expected in different scenarios and that it correctly interacts with
// its
// // dependencies
...
//     // the AuthService's register method returns a RegisterResponse containing the registered
// email
...
//     // this test checks that when a registration request is made with an email that already
// exists
//     // in the system,
...
//     // the method successfully verifies the token, updates the user's emailVerified status to
// true,
...
//     // token that has its verified property set to true, ensuring that the token is marked as
// used
...
//     // this test checks that when an email verification token that has already been used (i.e.
// its
...
//             .userId(activeUser.getId())
//             .expiresAt(
//                 LocalDateTime.now().plusHours(1)) // the token is not expired, 1 hour in the
// future
//             .verified(true)
//             .build();
...
//     // this test checks that when valid credentials are provided to the AuthService's login
// method,
...
//     // this checks that the token returned by the login method matches the mocked token generated
// by
//     // the jwtService,
...
//     // email, confirming that the response includes the expected user details for the
// authenticated
//     // user.
//   }
...
//     request.setEmail("bob@momentum.co.za");
//     request.setPassword(
//         "WrongPassword!"); // the password does not match the expected password for the user,
// which
//     // should trigger the invalid credentials error in the login method

//     when(userRepository.findByEmail("bob@momentum.co.za")).thenReturn(Optional.of(activeUser));
//     when(passwordEncoder.matches("WrongPassword!",
// activeUser.getPasswordHash())).thenReturn(false);
//     when(userRepository.save(any())).thenReturn(activeUser);
...
//     // AuthService's login method,
//     // the method throws an IllegalArgumentException with a message indicating invalid
// credentials,
//     // ensuring that the login process properly handles cases where the email is not found and
// does
...
//     // ensuring that the login process properly tracks failed login attempts and updates the
// user's
//     // account status
//     AuthRequest request = new AuthRequest();
//     request.setEmail("bob@momentum.co.za");
...
//     // this checks that when valid credentials are provided to the AuthService's login method,
// the
//     // method resets the user's loginAttempts count to 0,
//     // and returns a valid AuthResponse with a JWT token, ensuring that successful logins
// properly
...
//     // this test checks that when a JWT token string that includes the "Bearer " prefix is
// provided
//     // to the AuthService's logout method,
//     // the method correctly extracts the token from the string and calls the
...
//     // this test checks that when a JWT token string without the "Bearer " prefix is provided to
// the
//     // AuthService's logout method,
//     // the method directly calls the tokenBlacklistService's blacklistToken method with the
// provided
//     // token string,
//     // ensuring that the logout functionality correctly handles token strings without the prefix
// and
...
//     // the system and always returns a success message to prevent information disclosure about
// user
//     // accounts.
...
//     // ensuring that the forgot password functionality properly handles valid requests and
// initiates
...
//     verify(emailService).sendPasswordResetEmail(eq("bob@momentum.co.za"), eq("Bob"),
// anyString());
//   }
...
//   //     // this test checks that when a valid email that exists in the system is provided to the
//   //     // AuthService's forgotPassword method,
//   //     // the method still returns a MessageResponse with a message indicating that a reset
// link
//   // has
//   //     // been sent, without throwing an exception,
