...
//this integration test class verifies the functionality of the InsightsService's getInsightsSummary method,
//by mocking the TimeEntryRepository to return specific time entries for a user within a given date
//range, and asserting that the resulting InsightsSummaryResponse contains the correct total hours logged,
//total days logged, hours per project, hours per task, daily trend, and average hours per day,
//ensuring that the insights summary is accurately calculated based on the time entries retrieved from the repository.
...
import static org.assertj.core.api.Assertions.*; //assertj for fluent assertions
...
class InsightsServiceTest {
    ...
    private TimeEntry entry(int durationMinutes, LocalDateTime start) {
        //Helper method to create a TimeEntry with specified duration and start time, using fixed project and task IDs
    ...
    void getInsightsSummary_returnsZerosForNoEntries() {
        //this test checks that when the TimeEntryRepository returns an empty list of time entries for a user within a specified date range,
        //the InsightsService's getInsightsSummary method returns an InsightsSummaryResponse with total hours logged equal to 0.0,
    ...
    void getInsightsSummary_correctlyTotalsHours() {
        ...
        when(timeEntryRepository.findByUserIdAndDateRange(any(), any(), any()))
            .thenReturn(List.of(entry(480, day1), entry(300, day2))); // 8 hours on day 1 and 5 hours on day 2
    ...
    void getInsightsSummary_groupsByProject() {
        //this test checks that when the TimeEntryRepository returns multiple time entries for a user that belong to the same project,
        //the InsightsService's getInsightsSummary method correctly groups the hours by project in the resulting InsightsSummaryResponse,
        //by asserting that the hours per project list contains a single entry with the correct total hours and entry count for that project, ensuring that the grouping logic in the service works as
    ...
    void getInsightsSummary_groupsByTask() {
        //this test checks that when the TimeEntryRepository returns multiple time entries for a user that belong to the same task,
        //the InsightsService's getInsightsSummary method correctly groups the hours by task in the resulting InsightsSummaryResponse,
        //by asserting that the hours per task list contains a single entry with the
        //correct total hours and task ID for that task, ensuring that the grouping logic in the service works as expected for tasks
    ...
    void getInsightsSummary_dailyTrendSortedByDate() {
        //this test checks that when the TimeEntryRepository returns multiple time entries for a user on different dates,
        //the InsightsService's getInsightsSummary method correctly calculates the daily trend and sorts it by
        //date in ascending order in the resulting InsightsSummaryResponse, 
        //by asserting that the daily trend list contains entries with the correct dates in the expected order,
        //ensuring that the date sorting logic in the service works as intended for the daily trend calculation
    ...
    void getInsightsSummary_calculatesAverageHoursPerDay() {
        //this test checks that when the TimeEntryRepository returns time entries for a user that span multiple days,
        //the InsightsService's getInsightsSummary method correctly calculates the average hours per day in the
        //resulting InsightsSummaryResponse, by asserting that the average hours per day is equal to the total hours logged divided by the total days logged,
        //ensuring that the average hours per day calculation in the service is accurate based on the time entries retrieved from the repository
    ...
    void getInsightsSummary_skipsEntriesWithNullTaskId() {
        //this test checks that when the TimeEntryRepository returns a time entry for a user that has a null task ID,
        //the InsightsService's getInsightsSummary method correctly skips that entry when calculating hours per task
        //in the resulting InsightsSummaryResponse, by asserting that the hours per task list does not contain an entry for the null task ID,
        //while still including the hours for that entry in the total hours logged and hours per project calculations,
        //ensuring that the service properly handles time entries with null task IDs without causing errors
    ...
    void getInsightsSummary_passesCorrectDateRangeToRepository() {
        ...
        //with the appropriate start and end times, and calls the TimeEntryRepository's findByUserIdAndDateRange method with
        //the correct user ID and date range parameters, ensuring that the service properly
        //translates the request's date range into the format expected by the repository when retrieving time entries
        when(timeEntryRepository.findByUserIdAndDateRange(any(), any(), any()))
            .thenReturn(Collections.emptyList());

        insightsService.getInsightsSummary(request, user);

        verify(timeEntryRepository).findByUserIdAndDateRange(
            eq(user.getId()),
