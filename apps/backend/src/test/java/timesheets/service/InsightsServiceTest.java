...
// // by mocking the TimeEntryRepository to return specific time entries for a user within a given
// date
// // range, and asserting that the resulting InsightsSummaryResponse contains the correct total
// hours
// // logged,
// // total days logged, hours per project, hours per task, daily trend, and average hours per day,
// // ensuring that the insights summary is accurately calculated based on the time entries
// retrieved
// // from the repository.
...
//     // this test checks that when the TimeEntryRepository returns an empty list of time entries
// for
//     // a user within a specified date range,
//     // the InsightsService's getInsightsSummary method returns an InsightsSummaryResponse with
// total
...
//     when(timeEntryRepository.findByUserIdAndDateRange(any(), any(), any()))
//         .thenReturn(
//             List.of(entry(480, day1), entry(300, day2))); // 8 hours on day 1 and 5 hours on day
// 2
...
//     // this test checks that when the TimeEntryRepository returns multiple time entries for a
// user
//     // that belong to the same project,
//     // the InsightsService's getInsightsSummary method correctly groups the hours by project in
// the
//     // resulting InsightsSummaryResponse,
//     // by asserting that the hours per project list contains a single entry with the correct
// total
//     // hours and entry count for that project, ensuring that the grouping logic in the service
// works
//     // as
...
//     // this test checks that when the TimeEntryRepository returns time entries for a user that
// span
//     // multiple days,
...
//     // ensuring that the average hours per day calculation in the service is accurate based on
// the
...
//     // this test checks that when the TimeEntryRepository returns a time entry for a user that
// has a
//     // null task ID,
//     // the InsightsService's getInsightsSummary method correctly skips that entry when
// calculating
//     // hours per task
//     // in the resulting InsightsSummaryResponse, by asserting that the hours per task list does
// not
