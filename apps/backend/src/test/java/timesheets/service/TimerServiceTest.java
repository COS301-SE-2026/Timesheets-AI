...
class TimerServiceTest {
  ...
  @Mock private TimerSessionRepository timerSessionRepository; // fake timer DB

  @Mock private TimeEntryRepository timeEntryRepository; // fake time entry DB

  @Mock private WorkspaceMemberRepository workspaceMemberRepository; // fake member DB

  @Mock private ProjectRepository projectRepository; // fake project DB

  @Mock private TaskRepository taskRepository; // fake task DB

  @Mock private ProjectMemberRepository projectMemberRepository; // fake project-member DB
  ...
  @BeforeEach // JUnit will run before every test method, it will set new data so that the data does
  ...
  void startTimer_Success_ShouldCreateAndReturnTimer() {
    ...
    when(timerSessionRepository.findFirstByWorkspaceMemberIdInAndIsRunningTrue(any()))
        .thenReturn(Optional.empty()); // no active timer exists
    when(projectRepository.findById(projectId)).thenReturn(Optional.of(project));
    when(projectMemberRepository.existsByProjectIdAndWorkspaceMemberId(
            projectId, workspaceMemberId))
        .thenReturn(true); // user is assigned to project
  ...
  void startTimer_WorkspaceMemberNotFound_ShouldThrowException() {
    when(workspaceMemberRepository.findById(workspaceMemberId))
        .thenReturn(Optional.empty()); // member doesn't exist

    // assertThatThrownBy = AssertJ - asserts that an exception is thrown
    assertThatThrownBy(() -> timerService.startTimer(workspaceMemberId, request))
        .isInstanceOf(ResourceNotFoundException.class)
        .hasMessageContaining("Workspace member not found");
  }
  ...
  void startTimer_ActiveTimerExists_ShouldThrowConflictException() {
    ...
    when(workspaceMemberRepository.findAllByUserId(userId)).thenReturn(List.of(workspaceMember));
    when(timerSessionRepository.findFirstByWorkspaceMemberIdInAndIsRunningTrue(any()))
        .thenReturn(Optional.of(existingTimer)); // active timer found!

    assertThatThrownBy(() -> timerService.startTimer(workspaceMemberId, request))
        .isInstanceOf(ConflictException.class)
        .hasMessageContaining("Timer already active in another workspace");
  }
  ...
  void startTimer_ProjectNotFound_ShouldThrowException() {
    ...
    when(projectRepository.findById(projectId))
        .thenReturn(Optional.empty()); // project doesn't exist
  ...
  void startTimer_UserNotAssignedToProject_ShouldThrowUnauthorizedException() {
    ...
    when(projectMemberRepository.existsByProjectIdAndWorkspaceMemberId(
            projectId, workspaceMemberId))
        .thenReturn(false); // user not a member of this project!
  ...
  void startTimer_TaskDoesNotBelongToProject_ShouldThrowException() {
    ...
    when(taskRepository.findById(taskId))
        .thenReturn(Optional.of(wrongTask)); // task belongs to different project

    assertThatThrownBy(() -> timerService.startTimer(workspaceMemberId, request))
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining("Task does not belong to the specified project");
  }
  ...
  void stopTimer_NoActiveTimer_ShouldThrowException() {
    when(workspaceMemberRepository.findById(workspaceMemberId))
        .thenReturn(Optional.of(workspaceMember));
    when(timerSessionRepository.findByWorkspaceMemberIdAndIsRunningTrue(workspaceMemberId))
        .thenReturn(Optional.empty()); // no active timer found

    assertThatThrownBy(() -> timerService.stopTimer(workspaceMemberId))
  ...
  void discardTimer_NoActiveTimer_ShouldThrowException() {
    ...
    assertThatThrownBy(() -> timerService.discardTimer(workspaceMemberId))
        .isInstanceOf(IllegalStateException.class)
        .hasMessageContaining("No active timer found to discard");
  }
  ...
  void getActiveTimer_WhenTimerExists_ShouldReturnTimer() {
    ...
    when(timerSessionRepository.findByWorkspaceMemberIdAndIsRunningTrue(workspaceMemberId))
        .thenReturn(Optional.of(expectedTimer));
