...
// // allowing us to verify how the controller handles different inputs and service responses
// without
// // needing a full application context or real database interactions.
// // Each test method corresponds to a specific endpoint and scenario, ensuring comprehensive
// coverage
...
//         .andExpect(
//             jsonPath("$.message")
//                 .value("Verification email sent. Please check your inbox.")); // we dont have a
// real
...
//     // this test returns a 400 Bad Request status when the password is too weak in the
// registration
//     // payload,
...
//     // this test returns a 400 Bad Request status when the first name is missing in the
// registration
//     // payload,
...
//     // this test returns a 400 Bad Request status when the email already exists in the
// registration
//     // payload,
...
//     // the controller should respond with a 400 status when an attempt is made to register with
// an
...
//         .perform(
//             post("/api/auth/login")
//                 // this simulates an HTTP POST request to the /api/auth/login endpoint with a
// JSON
...
//     when(authService.login(any())).thenThrow(new IllegalArgumentException("invalid
// credentials"));
...
//     // this test returns a 400 Bad Request status when the email is missing from the login
// request,
...
//     // this test returns a 400 Bad Request status when the account is locked due to too many
// failed
//     // login attempts,
...
//   // these tests check the behavior of the logout endpoint, ensuring that it correctly handles
// valid
//   // tokens and missing authorization headers.
//   // The first test verifies that a valid token results in a successful logout with a 204 No
// Content
...
//     // this test returns a 204 No Content status when a valid token is provided in the
// Authorization
//     // header,
...
//     // this test returns a 400 Bad Request status when the Authorization header is missing from
// the
//     // logout request,
...
//     // and it checks that the response message indicates that a password reset link has been sent
// to
...
// to
//     // the email if the account exists,
//     // ensuring that the endpoint does not reveal whether the email is registered or not, which
// is a
