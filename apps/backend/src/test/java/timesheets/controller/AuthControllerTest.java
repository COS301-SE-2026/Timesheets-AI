...
//this unit test class focuses on testing the AuthController's endpoints and their expected behaviors under various scenarios.
//It uses MockMvc to simulate HTTP requests and Mockito to mock the AuthService's behavior,
//allowing us to verify how the controller handles different inputs and service responses without needing a full application context or real database interactions.
//Each test method corresponds to a specific endpoint and scenario, ensuring comprehensive coverage of the authentication flow.
...
import static org.mockito.ArgumentMatchers.any; //allows us to specify that any instance of the given type is acceptable when stubbing methods with Mockito
import static org.mockito.Mockito.*; //provides static methods for creating and configuring mock objects, as well as verifying interactions with those mocks in our tests
...
class AuthControllerTest {
    ...
    void register_returns201WithValidPayload() throws Exception {
        ...
        //and it checks that the response contains the correct email and a message indicating that a verification email has been sent.
        ...
        mockMvc.perform(post("/api/auth/register")
            ...
            .andExpect(jsonPath("$.message").value("Verification email sent. Please check your inbox.")); //we dont have a real email service to send the verification email, so we just return a generic message for testing purposes.
    }

    @Test
    void register_returns400WithInvalidEmailFormat() throws Exception { 
        //this test returns a 400 Bad Request status when the email format is invalid in the registration payload,
    ...
    void register_returns400WithWeakPassword() throws Exception {
        //this test returns a 400 Bad Request status when the password is too weak in the registration payload,
    ...
    void register_returns400WithMissingFirstName() throws Exception {
        //this test returns a 400 Bad Request status when the first name is missing in the registration payload,
    ...
    void register_returns400WhenEmailDomainNotAccepted() throws Exception {
        //this test returns a 400 Bad Request status when the email domain is not accepted in the registration payload,
        ...
        when(authService.register(any())).thenThrow(new IllegalArgumentException("email domain not accepted"));
    ...
    void register_returns400WhenEmailAlreadyExists() throws Exception {
        //this test returns a 400 Bad Request status when the email already exists in the registration payload,
        ...
        //the controller should respond with a 400 status when an attempt is made to register with an email that is already in use,
    ...
    //including valid credentials, wrong passwords, missing email, and account lockout due to too many failed attempts.
    ...
    void login_returns200WithValidCredentials() throws Exception { 
        ...
        //this simulates an HTTP POST request to the /api/auth/login endpoint with a JSON payload containing the email and password,
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request)))
            .andExpect(status().isOk())
    ...
    void login_returns400WithMissingEmail() throws Exception {
        ...
        //since the email field is required for authentication, the controller should respond with a 400 status when it's not provided,
    ...
    void login_returns400WhenAccountLocked() throws Exception {
        //this test returns a 400 Bad Request status when the account is locked due to too many failed login attempts,
        //and it checks that the error message indicates the account is locked.
        AuthRequest request = new AuthRequest();
        request.setEmail("bob@momentum.co.za");
        request.setPassword("Password123!");

        when(authService.login(any())).thenThrow(new IllegalStateException("account locked after too many failed attempts"));
    ...
    //these tests check the behavior of the logout endpoint, ensuring that it correctly handles valid tokens and missing authorization headers.
    //The first test verifies that a valid token results in a successful logout with a 204 No Content status, while the second test ensures that a missing authorization header leads to a 400 Bad Request status.

    @Test
    @WithMockUser
    void logout_returns204WithValidToken() throws Exception {
        //this test returns a 204 No Content status when a valid token is provided in the Authorization header,
        //and it verifies that the AuthService's logout method is called with the correct token.
        doNothing().when(authService).logout(any());

        mockMvc.perform(post("/api/auth/logout")
                .header("Authorization", "Bearer some-valid-jwt"))
    ...
    void logout_returns400WithMissingAuthorizationHeader() throws Exception {
        //this test returns a 400 Bad Request status when the Authorization header is missing from the logout request,
    ...
    //these tests check the behavior of the forgot password endpoint, making sure that it always returns a 200 OK status regardless of whether the email exists in the system or not.
    //The first test verifies that a valid email results in a successful response with a message indicating that a password reset link has been sent,
    ...
    void forgotPassword_alwaysReturns200() throws Exception {
        ...
        request.setEmail("anyone@momentum.co.za");//this test returns a 200 OK status regardless of whether the email exists in the system or not,
        //and it checks that the response message indicates that a password reset link has been sent to the email if the account exists.
        //at the moment we dont have an email service to send the reset link, 
        //so we just return a generic message for both existing and non-existing emails to prevent email enumeration attacks.

        when(authService.forgotPassword(any()))
            .thenReturn(new MessageResponse("Password reset link sent to your email if the account exists"));
    ...
    void forgotPassword_returns200EvenForUnknownEmail() throws Exception {
        //this test returns a 200 OK status even when an unknown email address is provided in the forgot password request,
        //and it checks that the response message indicates that a password reset link has been sent to the email if the account exists, 
        //ensuring that the endpoint does not reveal whether the email is registered or not, which is a common security practice to prevent email enumeration attacks.
        ForgotPasswordRequest request = new ForgotPasswordRequest();
