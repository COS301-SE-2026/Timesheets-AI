...
//         when(timerService.startTimer(eq(workspaceMemberId), any(StartTimerRequest.class))).thenReturn(timer);

//         //SpringBoot: simulates an HTTP POST request to the controller
//         mockMvc.perform(post("/api/timers/start").header("X-Workspace-Member-Id", workspaceMemberId).contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(startRequest)))
...
//         // the header X-Workspace-Member-Id tells the backend which workspace member is making the request
...
//         when(timerService.startTimer(eq(workspaceMemberId), any(StartTimerRequest.class))).thenThrow(new ConflictException("Timer already active", "You already have a running timer", timerId));

//         // SpringBoot: simulates an HTTP POST request to the controller
//         mockMvc.perform(post("/api/timers/start").header("X-Workspace-Member-Id", workspaceMemberId).contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(startRequest))).andExpect(status().isConflict()).andExpect(jsonPath("$.message").value("You already have a running timer")).andExpect(jsonPath("$.activeTimerId").value(timerId.toString()));
//         // .andExpect() verifies the actual response matches what we expect, we expect that 409 conflict that I wrote
//     }
...
//         //SpringBoot: simulates an HTTP POST request to the controller
//         mockMvc.perform(post("/api/timers/stop").header("X-Workspace-Member-Id", workspaceMemberId)).andExpect(status().isOk());
...
//         mockMvc.perform(get("/api/timers/active").header("X-Workspace-Member-Id", workspaceMemberId)).andExpect(status().isOk()).andExpect(jsonPath("$.id").value(timerId.toString())).andExpect(jsonPath("$.active").value(true));
...
//         //SpringBoot: simulates an HTTP GET request to the controller
//         mockMvc.perform(get("/api/timers/active").header("X-Workspace-Member-Id", workspaceMemberId)).andExpect(status().isNoContent());
//         // .andExpect() verifies the actual response of a 204
//     }


//     // this will test when a user cancels a running timer, should return success since there is nothing to run
//     @Test
...
//         //SpringBoot: simulates an HTTP DELETE request to the controller
//         mockMvc.perform(delete("/api/timers/discard").header("X-Workspace-Member-Id", workspaceMemberId)).andExpect(status().isNotFound());
